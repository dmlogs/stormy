...
describe("requestHandler", function() {
    ...
    describe("#getAll()", function() {
        ...
        it('should get standardized successfully for all sources', function(done) {
            var mockService = new MockService(function(type, lat, long, success, error) {
                assert.equal('std', type);
                assert.equal(100, lat);
                assert.equal(200, long);
                success("test");
            ...
            var res = new MockRes(function(statusCode, data) {
                assert.equal(200, statusCode);
                assert.equal("test", data.service);
                done();
            });
            req.setLatLong(100, 200);
            requestHandler.getAll(req, res);
        });
        it('should get standardized errors for all sources', function(done) {
            var mockService = new MockService(function(type, lat, long, success, error) {
                assert.equal('std', type);
                assert.equal(100, lat);
                assert.equal(200, long);
            ...
            var res = new MockRes(function(statusCode, data) {
                assert.equal(200, statusCode);
                assert.equal("test", data.error.service);
                done();
            });
            req.setLatLong(100, 200);
            requestHandler.getAll(req, res);
